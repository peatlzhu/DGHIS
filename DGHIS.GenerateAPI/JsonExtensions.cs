...
namespace DGHIS.GenerateAPI
{
    public static class JsonExtensions
    {
        ...
        {
            json = Regex.Replace(json, "\\\\/Date\\((\\d+)\\)\\\\/", (match) => new DateTime(1970, 1, 1).AddMilliseconds(long.Parse(match.Groups[1].Value)).ToLocalTime().ToString("yyyy-MM-dd HH:mm:ss.fff"));
            return json;
        }
