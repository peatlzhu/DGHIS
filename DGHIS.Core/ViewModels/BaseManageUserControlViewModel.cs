...
{
    /// <summary>
    /// 管理功能控件(UserControl)ViewModel基类
    /// </summary>
    public abstract class BaseManageUserControlViewModel : BaseViewModel, INavigationAware
    {
        /// <summary>
        /// 管理功能控件(UserControl)ViewModel基类构造函数
        /// </summary>
        /// <param name="container">注入容器</param>
        public BaseManageUserControlViewModel(IContainerExtension container) : base(container)
        {
        ...
        /// <summary>
        /// 处理窗体或UserControl加载事件
        /// </summary>
        public virtual DelegateCommand<UserControl> LoadedCommand => new DelegateCommand<UserControl>((uc) =>
        {
            if (uc == null) throw new Exception("未获取到当前UserControl对象，请UserControl上实现Loaded命令！");
            var manager = Container.Resolve<UserControlManager>();
            var controls = manager.GetControls(uc.GetType().FullName);
            controls.ForEach(x =>
            {
                Navigate(x.RegionName, x.ControlType.FullName);
            });
            UserControlLoaded(uc);
        });
        ...
        /// </summary>
        /// <param name="uc"></param>
        public abstract void UserControlLoaded(UserControl uc);
            ...
            where TEntity : class, new()
        {
            if (CanHandle)
            {
                ...
                Confirm($"请确认是否修改{statusName}状态？", (d) =>
                {
                    ...
                    {
                        BindingOperations.GetBindingExpression(combox, System.Windows.Controls.Primitives.Selector.SelectedValueProperty).UpdateSource();
                        UpdateDataStatus(t);
                    }
                    else
                    {
